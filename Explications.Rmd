---
title: "Explications Turtle Trading"
author: "Tien-Thinh"
date: "2023-04-22"
output: pdf_document
---
...
Nous utilisons les données $BTC-USD$ de `quantmod`.\
Et pour appliquer la stratégie Turtle nous ajoutons :\
- Les Chaines de Donchian 20 jours (blanc, signal d'ouverture)\
- Les Chaines de Donchian 10 jours (bleu, signal de clôture)\
- L'Average True Range (ATR) 10 jours

```{r, fig.keep="last"}
# getSymbols BTC-USD into df
df <- getSymbols(Symbols = "BTC-USD", auto.assign = FALSE, from=as.Date("2020-01-01"), to=as.Date("2020-06-01"))
# Create chart Visualisation 
chartSeries(df, up.col="green",dn.col="red")
# Create Technical Analysis
dc20 <- DonchianChannel(HL(df), n=20)[,c("high", "low")]
dc10 <- DonchianChannel(HL(df), n=10)[,c("high", "low")]
atr <- ATR(HLC(df), n=14)$atr
# Add TA to chart
addTA(dc20, legend="Donchian 20", on=1, col=c("white", "white"))
addTA(dc10, legend="Donchian 10", on=1, col=c("deepskyblue", "deepskyblue"))
addTA(atr, legend="ATR", col="white")
```
  
# Création des ordres
Nous travaillons sur les données entre jenvier 2020 et janvier 2023.  
```{r, warning = FALSE}
# getSymbols BTC-USD into df  
...
getCumProd <- function(result, factor=1, fees=0.0006) {
  return (result %>%
    mutate(
      bEARN=(bEARN-fees) %>% na.fill(fill=1), sEARN=(sEARN-fees) %>% na.fill(fill=1)result,
...
# Choix du meilleur facteur risk 3
```{r, warning = FALSE}
# Curve earnpow
...
# Gain potentiel à facteur risk 3
```{r, warning = FALSE}
# CUMPROD with POW
result <- result %>% 
  getCumProd(factor=5)
...
x_long <- result %>% 
  rowid_to_column("x") %>% 
  melt(id.vars ="x", measure.vars = c("sCUMULpow", "bCUMULpow"))
ggplot(x_long, aes(x=x, y = value)) +
  geom_point(aes(color = variable)) +
  scale_y_continuous(trans='log10') +
  xlab("Date") +
...
result[nrow(result), c("sCUMULpow", "bCUMULpow")] %>% 
  paste("Profit : ", .) %>% 
  knitr::kable()
```
...
# Gain trade simultanés additionés
